...
double peak_value;
PeakFinder::findPeaks(histVal, peak_idxs, false);

if(peak_idxs.size() < 1){
    ROS_INFO("No lane detected -- NO PEAK");
    return;
}

std::vector<double> pkHistVal, startYs;

for (std::vector<int>::const_iterator it = peak_idxs.begin(); it != peak_idxs.end(); it++) {
    pkHistVal.push_back(histVal[*it]);
     startYs.push_back(yvals[*it]);
}
